...
def choose_background_injection_training(f, randomseed=1986, zero_noise=False):
    ...
    injection_events = [i for i in f.root.onsource_injection._v_children.keys()]

    waveforms = [waveform for waveform in f.get_node('/onsource_injection/{0}'.format(injection_events[0]))._v_children.keys()]

    inj_scales = [inj_scale for inj_scale in f.get_node('/onsource_injection/{0}/{1}'.format(injection_events[0], waveforms[0]))._v_children.keys()]

    injections = numpy.asarray([injection for injection in f.get_node('/onsource_injection/{0}/{1}/{2}'.format(injection_events[0], waveforms[0], inj_scales[0]))._v_children.keys()])

    training_injection_events_idx = numpy.random.randint(0, injections.size, size=int(0.5*injections.size))

    if zero_noise:
        list_of_training_injection_paths = [['/zero_noise_injection'], injection_events, waveforms,
                                            inj_scales, injections[training_injection_events_idx].tolist()]
    else:
        list_of_training_injection_paths = [['/onsource_injection'], injection_events, waveforms,
                                            inj_scales, injections[training_injection_events_idx].tolist()]

    training_injection_events = list(map(lambda x: '/'.join(x), itertools.product(*list_of_training_injection_paths)))

    list_of_validation_injection_paths = [['/onsource_injection'], injection_events, waveforms, inj_scales, injections[~training_injection_events_idx].tolist()]
