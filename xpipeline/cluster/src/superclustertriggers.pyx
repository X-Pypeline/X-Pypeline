...
def fastsupercluster_wrapper(ntriggers, object rectangles,):
    cdef:
        np.ndarray[np.double_t, ndim=1, mode='c'] rectangles_tmp
        np.ndarray[np.double_t, ndim=1, mode='c'] mask = np.zeros((ntriggers,), dtype=np.double)

    rectangles_tmp = np.ascontiguousarray(rectangles.flatten('F'), dtype=np.float64)

    fastsupercluster(ntriggers, &rectangles_tmp[0], &mask[0])
    return mask
