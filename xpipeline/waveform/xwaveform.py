...
def xmakewaveform(family, parameters, T, T0, fs, **kwargs):
    ...
    if family.lower() in ['aorddmj10', 'dojmm07', 'domj08', 'mob09', 'obdl06', 'oblw04']:
        ...
        h = h_rss * (
                     ...
                     / 
                     (2 * math.pi * tau**2)**(1/4))
        h = TimeSeries(h, dx=1./16384, name='chirplet')
    ...
    """
            ...
            h = 1./distance*DFMcat(DFM_number).hoft.T
            hp = h(:)
            hc = zeros(size(hp))

    elif family.lower() in ['dfmc']:
            ...
            trend = hnz(end)*([0:length(hnz)-1].T)/(length(hnz)-1)
            ht = hnz - trend
            trend = ht(1)*([length(hnz)-1:-1:0].T)/(length(hnz)-1)
            ht = ht - trend
            h(index) = ht
            #
            hp = h(:)
            ...
            h = 1./distance*DFM02cat(DFM02_number).hoft.T
            hp = h(:)
            hc = zeros(size(hp))
    ...
    elif family.lower() in ['iscochirpa','iscochirpb','iscochirpc']:
            ...
            # ---- Read waveform, which is defined at a range of 10kpc.
            #      Make sure h is same type of vector (column) as t.
            h = 10/parameters{1} * Marek2009cat(Marek2009_number).h.T
            hp = h(:)
            hc = zeros(size(hp))
    ...
    elif family.lower() in ['mono', 'mono-a', 'mono-b', 'mono-c', 'line-a', 'line-b', 'quad-a', 'quad-b']:
            ...
            hp = (catalog.strain{index}*hrss).T
            hc = zeros(size(hp))
            hb = zeros(size(hp))
            ...
            # ---- Load waveform structure, if not already loaded or supplied.
            catalog = scipy.io.loadmat(os.path.join(filedir, family.lower() + '.mat'))

            # ---- Find specified waveform type.
            distance = parameters[0]
            bns_name  = parameters[1]
            index = numpy.where(bns_name in catalog['bns_name'])[0]

            #      We must scale waveforms in catalog to have hrss=1 and pad
            #      are 1 second long at 16384 Hz.
            hp = numpy.zeros(pregen_fs)
            hc = numpy.zeros(pregen_fs)
            hb = numpy.zeros(pregen_fs)

            # ---- Read waveform.
            hp[:catalog['hphc'][index][0][0][0].size] = catalog['hphc'][index][0][0][0] / (distance/20.)
            hc[:catalog['hphc'][index][0][0][1].size] = catalog['hphc'][index][0][0][1] / (distance/20.)

            pregen_T = hp.size/pregen_fs
